            ...
            {if $group.attributes}
              <div class="from-group-option{if $group.required} required{/if}">
                {if $group.id_ets_eto_attr_group || Count($attribute_groups)>1}
                  <h3 class="h5">{$group.name|escape:'html':'UTF-8'} {if $group.required}<span class="required"></span> {/if}</h3>
                {/if}
                {if $group.type_option=='radio'}
                  {foreach from=$group.attributes item='attribute'}
                    <div class="radio my-3">
                      <label id="custom-attribute-{$attribute.id_ets_eto_attr|intval}" class="control-label fs-6">
                        ...
                        {if $attribute.image}<img class="border" src="{$attribute.image|escape:'html':'UTF-8'}" style="height: 32px; width: 32px;" /> {/if}{$attribute.name|escape:'html':'UTF-8'}
                        <span class="product-discount d-inline-flex">
                                                    (
