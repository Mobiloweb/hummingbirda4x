...
<div id="PM_ASFormContainerHidden" style="display: none">{/if}
  ...
  <div class="accordion accordion-flush" id="accordionFlushAS5-6">
    ...
    <div {if $as_search.id_search === 6 || $as_search.id_search === 13}id="flush-as5-6" class="accordion-collapse collapse"
      ...
      <form action="{$ASSearchUrlForm}" method="POST" id="PM_ASForm_{$as_search.id_search|intval}"
            class="PM_ASForm {if $page.page_name !== 'index'}position-relative{/if}">
        <div class="PM_ASCriterionsGroupList{if !preg_match('/.*(left|right)column$/i', $hookName)} row{/if}">
          {foreach from=$as_search.criterions_groups item=criterions_group name=criterions_groups}
              ...
              {if isset($as_search.selected_criterion[$criterions_group.id_criterion_group])}
                {foreach from=$as_search.selected_criterion[$criterions_group.id_criterion_group] item=selected_id_criterion name=selected_criteria}
                  {if !$criterions_group.visible}
                    <input type="hidden" name="as4c[{$criterions_group.id_criterion_group|intval}][]"
                           value="{$selected_id_criterion nofilter}"/>
                    <input type="hidden" name="as4c_hidden[{$criterions_group.id_criterion_group|intval}][]"
                           value="{$selected_id_criterion nofilter}"/>
                  {/if}
                {/foreach}
              {/if}
            ...
            {/if}
          {/foreach}
          {if $as_search.search_method == 2 || $as_search.search_method == 4 && $page.page_name !== 'index'}
            <p class="col-xs-12 text-center mt-4 submit-wrapper">
              <input type="submit" value="{l s='Search' mod='pm_advancedsearch4'}" name="submitAsearch"
        ...
        {if \AdvancedSearch\SearchEngineUtils::getCurrentCategory()}
          <input type="hidden" name="id_category_search"
                 value="{if isset($as_search.id_category_root) && $as_search.id_category_root > 0}{$as_search.id_category_root|intval}{else if \AdvancedSearch\SearchEngineUtils::getCurrentCategory()}{\AdvancedSearch\SearchEngineUtils::getCurrentCategory()|intval}{/if}"/>
        {/if}
        {if \AdvancedSearch\SearchEngineUtils::getCurrentManufacturer()}
          <input type="hidden" name="id_manufacturer_search"
                 value="{\AdvancedSearch\SearchEngineUtils::getCurrentManufacturer()|intval}"/>
        {/if}
        {if \AdvancedSearch\SearchEngineUtils::getCurrentSupplier()}
          <input type="hidden" name="id_supplier_search"
                 value="{\AdvancedSearch\SearchEngineUtils::getCurrentSupplier()|intval}"/>
        {/if}
        {if \AdvancedSearch\SearchEngineUtils::getCurrentCMS()}
          <input type="hidden" name="id_cms_search"
                 value="{\AdvancedSearch\SearchEngineUtils::getCurrentCMS()|intval}"/>
        {/if}
        {if $as_search.step_search}
          <input type="hidden" name="next_id_criterion_group" value=""/>
        {/if}
        <input type="hidden"
               name="orderby"{if isset($smarty.get.order) && $smarty.get.order} value="{$smarty.get.order}"{else} disabled="disabled"{/if} />
        <input type="hidden"
               name="n"{if isset($smarty.get.n) && $smarty.get.n} value="{$smarty.get.n|intval}"{else} disabled="disabled"{/if} />
        {if $as_search.search_method == 2 || $as_search.search_method == 4 && $page.page_name === 'index'}
          <p class="col-xs-12 text-center mt-4 submit-wrapper">
            <input type="submit" value="{l s='Search' mod='pm_advancedsearch4'}" name="submitAsearch"
                   class="btn btn-primary PM_ASSubmitSearch text-white text-uppercase py-2"/></p>
