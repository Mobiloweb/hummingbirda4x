  ...
  {block name='cart_summary_tax'}
    {if $cart.subtotals.tax}
      <div class="cart-summary__line">
        <span class="cart-summary__label cart-summary__label--sub fw-normal">{l s='%label%:' sprintf=['%label%' => $cart.subtotals.tax.label] d='Shop.Theme.Global'}</span>
        <span class="cart-summary__value cart-summary__label--sub fw-normal">{$cart.subtotals.tax.value}</span>
      </div>
    {/if}
  {/block}
